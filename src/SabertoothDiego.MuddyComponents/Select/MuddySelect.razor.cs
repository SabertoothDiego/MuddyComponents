...
public partial class MuddySelect<T> : MudBaseInput<T>, IMuddySelect, IMuddyShadowSelect
{
    ...
    [Inject]
    protected IKeyInterceptorFactory KeyInterceptorFactory { get; set; }
    [Inject]
    protected IScrollManager ScrollManager { get; set; }

    protected string _elementId = $"select_{Guid.NewGuid().ToString()[..8]}";

    protected Task SelectNextItem() => SelectAdjacentItem(+1);

    protected Task SelectPreviousItem() => SelectAdjacentItem(-1);

    protected async Task SelectAdjacentItem(int direction)
    {
    ...
    protected ValueTask ScrollToItemAsync(MuddySelectItem<T> item)
        => item != null ? ScrollManager.ScrollToListItemAsync(item.ItemId) : ValueTask.CompletedTask;
    protected async Task SelectFirstItem(string startChar = null)
    {
        if (_items == null || _items.Count == 0)
            return;
    ...
    protected async Task SelectLastItem()
    {
        if (_items == null || _items.Count == 0)
            return;
    ...
    [Parameter]
    public Func<bool, bool> OnBeforeFocusLost { get; set; }
    ...
    [Parameter]
    public RenderFragment PreListContent { get; set; }

    [Parameter]
    public RenderFragment PostListContent { get; set; }
    ...
    protected internal event Action<ICollection<T>> SelectionChangedFromOutside;
    ...
    /// </summary>
    public IReadOnlyList<MuddySelectItem<T>> Items => _items;

    protected internal List<MuddySelectItem<T>> _items = new();
    protected Dictionary<T, MuddySelectItem<T>> _valueLookup = new();
    protected Dictionary<T, MuddySelectItem<T>> _shadowLookup = new();

    // note: this must be object to satisfy MudList
    private object _activeItemId = null;

    protected internal bool Add(MuddySelectItem<T> item)
    {
        if (item == null)
            return false;
        bool? result = null;
    ...
    protected internal void Remove(MuddySelectItem<T> item)
    {
        _items.Remove(item);
        if (item.Value != null)
    ...
    protected async void HilightItemForValue(T value)
    {
        if (value == null)
        {
            HilightItem(null);
    ...
    protected async void HilightItem(MuddySelectItem<T> item)
    {
    ...
    protected async Task HilightSelectedValue()
    {
        await WaitForRender();
        if (MultiSelection)
    ...
    protected void UpdateSelectAllChecked()
    {
    ...
    protected virtual void HandleKeyUp(KeyboardEventArgs obj)
    {
    ...
    protected async Task SelectAllClickAsync()
    {
    ...
    public void RegisterShadowItem(MuddySelectItem<T> item)
    {
    ...
    public void UnregisterShadowItem(MuddySelectItem<T> item)
    {
        if (item == null || item.Value == null)
            return;
    ...
    protected void OnLostFocus(FocusEventArgs obj)
    {
        var shouldLooseFocus = true;
        if (OnBeforeFocusLost is not null) shouldLooseFocus = !OnBeforeFocusLost(_isOpen);
        if (_isOpen && shouldLooseFocus)
        {
